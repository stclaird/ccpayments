...
/**
Prestas Shop OffLine Credit Card Payments "ccpayments"
This module is designed to work with Prestashop Version 1.6
	...
	public function install()
	{
		if (!parent::install()
			...
			OR !Configuration::updateValue('CCPAYMENTCARD_CVC_ENABLED', '1')
                        OR !Configuration::updateValue('CCPAYMENTCARD_ISSUE_ENABLED', '1')
	...
	function uninstall()
        {
            return (
                    ...
                    Configuration::deleteByName('CCPAYMENTCARD_CVC_ENABLED') AND
                    !$this->dropPaymentcardtbl() //calls function to drop payment card table
                   );
        }
...
public function getValidCards()
	{
		...
		{$valid_card = '<option value="Visa" >Visa</option>';}
		if (Configuration::get('CCPAYMENTCARD_MCARD_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="MasterCard" >Mastercard</option>';}
		if (Configuration::get('CCPAYMENTCARD_AMEX_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="AmEx" >American Express</option>';}
		if (Configuration::get('CCPAYMENTCARD_SWIT_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="Switch">Switch/Delta</option>';}
		if (Configuration::get('CCPAYMENTCARD_DISC_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="Discover">Discover</option>';}
		if (Configuration::get('CCPAYMENTCARD_JCB_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="Jcb">JCB</option>';}
		if (Configuration::get('CCPAYMENTCARD_LASE_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="Laser">Laser</option>';}
		if (Configuration::get('CCPAYMENTCARD_SOLO_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="Solo">Solo</option>';}
		if (Configuration::get('CCPAYMENTCARD_DINE_ENABLED')== "1")
		{$valid_card = $valid_card.'<option value="Diners">Diners</option>';}
	...
	*/
        ...
        function hookadminOrder($params){
            ...
            $id_order = $params['id_order']; //grab the order ID
                ...
                if($this->isPaymentCardOrder($id_order)){
                    ...
                    $payment_card_details = $this->readPayment($id_order);
                    $card_type = $this->bfish->decrypt($payment_card_details['card_type']);
                    $card_holder_name = $this->bfish->decrypt($payment_card_details['card_holder_name']);
                    $card_number = $this->bfish->decrypt($payment_card_details['card_number']);		//Decrypt cardNumber before displaying in invoice
                    $card_cvc = $this->bfish->decrypt($payment_card_details['card_cvc']);
                    $card_exp = $this->bfish->decrypt($payment_card_details['card_exp']);
                    $card_start = $this->bfish->decrypt($payment_card_details['card_start']);
                    $card_issue = $this->bfish->decrypt($payment_card_details['card_issue']);
                ...
                return $this->display(__FILE__, 'views/invoice_block.tpl');
	...
	function createPaymentcardtbl()
	{
		     ...
		     * card_holder_name: Stores the card holder name
		     ...
		     */
					...
					`card_start` TEXT DEFAULT NULL,
					`card_issue` INT DEFAULT NULL
					)";
    		        $db->Execute($query);
			return true;
	}
        ...
        /*
        *  Call this function to delete the payment card table
        */
        function dropPaymentcardtbl() {
            $db = Db::getInstance(); 
            		$query = "DROP TABLE `"._DB_PREFIX_."order_paymentcard`";
    		        $db->Execute($query);
			return true;
        }
	...
	*/
	...
	{
		$removed_string = pSQL($this->bfish->encrypt("Credit Card Information Has Been Removed."), true);
		$db = Db::getInstance();
		$result = $db->Execute('
		UPDATE `'._DB_PREFIX_.'order_paymentcard`
		SET 
                `card_type` = "'.$removed_string.'",
                `card_holder_name` = "'.$removed_string.'",
                `card_number` = "'.$removed_string.'",
                `card_cvc` = "'.$removed_string.'",
                `card_exp` = "'.$removed_string.'",
                `card_start` = "'.$removed_string.'",   
                `card_issue` = "'.$removed_string.'"
		WHERE `id_order` = "'.intval($id_order).'"');
	}
